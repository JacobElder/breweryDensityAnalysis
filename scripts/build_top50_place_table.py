"""Render a styled PNG table of the top 50 places (cities/towns) by brewery density.
...
has a floored variant — raw rates are dominated by small-place noise.
"""
...
def main() -> None:
    df = pd.read_parquet("data/processed/us_place_analysis.parquet")
    top = (
    ...
    col_labels = ["Rank", "Place", "Breweries", "Adults 21+", "Raw rate\n/100k", "Shrunken rate\n/100k"]
    ...
    fig.suptitle(f"Top {TOP_N} US Places (Cities/Towns) by Brewery Density", fontsize=16, fontweight="bold", y=0.99)
    fig.text(0.5, 0.955,
    ...
    out_path = "data/processed/us_top50_place_brewery_density_table.png"
