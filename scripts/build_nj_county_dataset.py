"""Assemble the New Jersey county-level analysis dataset and run validation checkpoints 1-3.

Calibration state (following NC, MI, CO, OR, WA, TX, GA, WI, PA, IL, CA, NY, VA, NE).
Liquor-registry source is breweries.sources.nj_liquor -- the NJ Division of
Alcoholic Beverage Control's own monthly "Wholesale/State Issued Licensee
Listing" Excel export, filtered to the three brewery license types (Plenary
Brewery, Limited Brewery, Restricted Brewery) (see that module's docstring
for the full inclusion-rule writeup). NJ's export has no county field, only
street/city/zip, so -- like WA's WSLCB list -- it is run through
breweries.geocode.fill_missing_coords (the Census Geocoder fallback) before
the county spatial join.
"""
...
from breweries.sources import acs, cbp, nj_liquor, obdb, osm

pd.set_option("display.width", 160)

# Checked https://www.brewersassociation.org/statistics-and-data/state-craft-beer-stats/
# on 2026-08-31 (single state-total lookup, no directory scrape): New Jersey has 159
# craft breweries, ranked #20 nationally.
BA_NJ_TOTAL_2025 = 159  # checked 2026-08-31, single state-total lookup


def build_obdb_county_counts() -> pd.DataFrame:
    df = obdb.load_state("New Jersey")
    df = obdb.apply_inclusion_rule(df, "obdb_nj")
    df = fill_missing_coords(df, "id", "latitude", "longitude", "address_1", "city",
                              "state_province", "postal_code", "obdb_nj")
    geo = assign_geographies(df, "latitude", "longitude", "NJ", "obdb_nj")
...
def build_osm_county_counts() -> pd.DataFrame:
    df = osm.load_state("NJ")
    geo = assign_geographies(df, "lat", "lon", "NJ", "osm_nj")
...
def build_liquor_county_counts() -> pd.DataFrame:
    df = nj_liquor.load()
    df = fill_missing_coords(df, "nj_liquor_id", "lat", "lon", "street_address", "city",
                              "state", "zip", "nj_liquor")
    geo = assign_geographies(df, "lat", "lon", "NJ", "nj_liquor")
...
def main() -> None:
    ...
    out_path = Path("data/processed/nj_county_analysis.parquet")
    ...
    print("=" * 70)
    print("CHECKPOINT 1: Face validity (Ocean/Monmouth shore counties and Camden/")
    print("Burlington near Philadelphia should rank high in NJ's dense craft-brewery belt)")
    print("=" * 70)
    ...
    for label, val in [
        ...
        ("NJ ABC (Plenary/Limited/Restricted Brewery, deduped)", df["liquor_count"].sum()),
        ("Brewers Association (2025)", BA_NJ_TOTAL_2025),
    ]:
        capture = val / BA_NJ_TOTAL_2025 * 100
